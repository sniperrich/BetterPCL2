...
internal static class SvgIconParser
{
    ...
    {
        double cx = _Number(element, "cx");
        double cy = _Number(element, "cy");
        double radius = _Number(element, "r");
        var geometry = new EllipseGeometry(new Rect(cx - radius, cy - radius, radius * 2d, radius * 2d));
    ...
    private static SvgIconElement _CreateEllipse(XElement element, SvgIconStyle style)
    {
        double cx = _Number(element, "cx");
        double cy = _Number(element, "cy");
        double rx = _Number(element, "rx");
        double ry = _Number(element, "ry");
    ...
    private static StreamGeometry? _CreatePointsGeometry(string? points, bool close)
    {
        ...
        using (var context = geometry.Open())
        {
            ...
            for (var i = 2; i + 1 < numbers.Length; i += 2)
                context.LineTo(new Point(numbers[i], numbers[i + 1]), true);
