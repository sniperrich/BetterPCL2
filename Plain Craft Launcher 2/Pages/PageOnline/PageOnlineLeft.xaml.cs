...
public partial class PageOnlineLeft : IRefreshable
{
    ...
    private FormMain.PageSubType pageID = FormMain.PageSubType.OnlineNeteaseProxy;
    ...
    private void PageCheck(object senderRaw, ModBase.RouteEventArgs e)
    {
        ...
        {
            var target = (FormMain.PageSubType)ModBase.Val(sender.Tag);
            if (RequiresOnlineAccount(target) && !PCL.Online.OnlineAccountService.IsLoggedIn)
            {
                ModMain.Hint(PCL.Core.App.Localization.Lang.Text("Online.Login.Required"), ModMain.HintType.Critical);
                SelectCurrentItem();
                return;
            }
    ...
    public object PageGet(FormMain.PageSubType? id = null)
    {
        ...
        if (!pages.TryGetValue(targetId, out var page))
        {
            page = targetId switch
            {
                ...
                FormMain.PageSubType.OnlineNeteaseProxy => new PageOnlineNeteaseProxy(),
    ...
    private static bool RequiresOnlineAccount(FormMain.PageSubType pageType)
    {
        return pageType != FormMain.PageSubType.OnlineNeteaseProxy;
    }
}
