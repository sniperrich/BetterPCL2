...
using PCL.Network;
using PortableLog = PclPortable::PCL.Core.Logging.PortableLog;
using PortableLogEntry = PclPortable::PCL.Core.Logging.PortableLogEntry;
using PortableLogLevel = PclPortable::PCL.Core.Logging.PortableLogLevel;
...
public partial class FormMain
{
    ...
    private void BtnNavItem_Click(object sender, MouseButtonEventArgs e)
    {
        ...
        if (pageType == PageType.Online &&
            (ModMain.frmOnlineLeft?.PageID ?? PageSubType.OnlineNeteaseProxy) != PageSubType.OnlineNeteaseProxy &&
            !PCL.Online.OnlineAccountService.IsLoggedIn)
        {
            e.Handled = true;
    ...
    {
        var effectiveOnlineSubType = subType != PageSubType.Default
            ? subType
            : ModMain.frmOnlineLeft?.PageID ?? PageSubType.OnlineNeteaseProxy;
        if (stack.page == PageType.Online &&
            effectiveOnlineSubType != PageSubType.OnlineNeteaseProxy &&
            !PCL.Online.OnlineAccountService.IsLoggedIn)
        {
            ModMain.Hint(Lang.Text("Online.Login.Required"), ModMain.HintType.Critical);
            PageChange(PageType.Setup, PageSubType.SetupOnline);
