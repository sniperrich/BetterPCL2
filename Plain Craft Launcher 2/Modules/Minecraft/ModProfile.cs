...
public static class ModProfile
{
    ...
    public class McProfile
    {
        ...
        public string CommunityLoginKind;
    ...
    public static void GetProfile()
    {
        ...
        try
        {
            ...
            foreach (var Profile in profileListJobj)
            {
                ...
                else if ((string)Profile["type"] == "4399")
                    newProfile = new McProfile
                    {
                        Type = ModLaunch.McLoginType._4399,
                        Uuid = (string)Profile["uuid"],
                        Username = (string)Profile["username"],
                        AccessToken = EncryptHelper.SecretDecrypt((string?)Profile["accessToken"]),
                        Name = EncryptHelper.SecretDecrypt((string?)Profile["name"]),
                        Password = EncryptHelper.SecretDecrypt((string?)Profile["password"]),
                        CommunityEntityId = (string?)Profile["communityEntityId"] ?? "",
                        CommunityLoginKind = (string?)Profile["communityLoginKind"] ?? "",
                        CommunityDetailsJson = EncryptHelper.SecretDecrypt((string?)Profile["communityDetailsJson"]),
                        Desc = (string)Profile["desc"],
                        SkinHeadId = (string)Profile["skinHeadId"]
                    };
                else if ((string)Profile["type"] == "netease")
                    newProfile = new McProfile
                    {
                        Type = ModLaunch.McLoginType.NetEase,
                        Uuid = (string)Profile["uuid"],
                        Username = (string)Profile["username"],
                        AccessToken = EncryptHelper.SecretDecrypt((string?)Profile["accessToken"]),
                        Name = EncryptHelper.SecretDecrypt((string?)Profile["name"]),
        ...
        try
        {
            ...
            else
            {
                ...
                foreach (var Profile in profileList)
                {
                    ...
                    else if (Profile.Type == ModLaunch.McLoginType._4399)
                        profileJobj = new JsonObject
                        {
                            { "type", "4399" }, { "uuid", Profile.Uuid }, { "username", Profile.Username },
                            { "accessToken", EncryptHelper.SecretEncrypt(Profile.AccessToken) },
                            { "name", EncryptHelper.SecretEncrypt(Profile.Name) },
                            { "password", EncryptHelper.SecretEncrypt(Profile.Password) },
                            { "communityEntityId", Profile.CommunityEntityId },
                            { "communityLoginKind", Profile.CommunityLoginKind },
                            { "communityDetailsJson", EncryptHelper.SecretEncrypt(Profile.CommunityDetailsJson) },
                            { "desc", Profile.Desc }, { "skinHeadId", Profile.SkinHeadId }
                        };
                    else if (Profile.Type == ModLaunch.McLoginType.NetEase)
                        profileJobj = new JsonObject
                        {
                            { "type", "netease" }, { "uuid", Profile.Uuid }, { "username", Profile.Username },
                            { "accessToken", EncryptHelper.SecretEncrypt(Profile.AccessToken) },
                            { "name", EncryptHelper.SecretEncrypt(Profile.Name) },
                            { "password", EncryptHelper.SecretEncrypt(Profile.Password) },
                            { "communityEntityId", Profile.CommunityEntityId },
    ...
    public static void CreateProfile()
    {
        ...
        else if (selectedAuthTypeNum.HasValue && selectedAuthTypeNum.Value == 2) // 4399 验证
            ModBase.RunInUi(() => ModMain.frmLaunchLeft.RefreshPage(true, ModLaunch.McLoginType._4399));
        else if (selectedAuthTypeNum.HasValue && selectedAuthTypeNum.Value == 3) // 网易验证
            ModBase.RunInUi(() => ModMain.frmLaunchLeft.RefreshPage(true, ModLaunch.McLoginType.NetEase));
        else // 离线验证
...
if (profile.Type == ModLaunch.McLoginType.Ms)
    ...
    public static object GetProfileInfo(McProfile profile)
    {
        ...
        {
            info += "4399";
            info += $" / {GetCommunityMethodLabel(profile)}";
        }
        else if (profile.Type == ModLaunch.McLoginType.NetEase)
        {
            info += "Netease";
    ...
    public static ModLaunch.McLoginData GetLoginData(ModLaunch.McLoginType targetAuthType = default)
    {
        ...
        if (selectedProfile is null) // 新档案
        {
            ...
            if (authType == ModLaunch.McLoginType._4399)
                return new ModLaunch.McLogin4399
                {
                    UserName = "",
                    Password = "",
            ...
            if (authType == ModLaunch.McLoginType.NetEase)
                return new ModLaunch.McLoginNetEase
                {
                    UserName = "",
                    Password = "",
                    CommunityLoginKind = OpenNelLoginKind.Unknown
                };
        ...
        if (authType == ModLaunch.McLoginType._4399)
            return new ModLaunch.McLogin4399
            {
                ...
                CommunityEntityId = selectedProfile.CommunityEntityId ?? "",
                CommunityDetailsJson = GetCommunityDetailsJson(selectedProfile),
                CommunityLoginKind = GetCommunityLoginKind(selectedProfile)
            };
    ...
    public static string IsProfileValid()
    {
        switch (selectedProfile.Type)
        {
            ...
            case ModLaunch.McLoginType._4399:
            {
                if (string.IsNullOrEmpty(selectedProfile.Name?.Trim()))
                    return "4399 登录：用户名或手机号不能为空";
                return "";
            }
            case ModLaunch.McLoginType.NetEase:
            {
                if (string.IsNullOrEmpty(selectedProfile.Name?.Trim()))
                    return "Netease 登录：用户名或手机号不能为空";
                return "";
            }
        }
    ...
    private static string GetCommunityMethodLabel(McProfile profile)
    {
        return GetCommunityLoginKind(profile) switch
        {
            OpenNelLoginKind.Login4399Password => "账号密码",
            OpenNelLoginKind.NeteaseCookie => "Cookie",
            OpenNelLoginKind.NeteasePhone => "手机号验证码",
            OpenNelLoginKind.NeteaseEmail => "邮箱密码",
            _ => "未识别"
        };
    }

    private static OpenNelLoginKind GetCommunityLoginKind(McProfile profile)
    {
        if (profile is null)
            return OpenNelLoginKind.Unknown;

        if (!string.IsNullOrWhiteSpace(profile.CommunityLoginKind) &&
            Enum.TryParse(profile.CommunityLoginKind, true, out OpenNelLoginKind parsed))
            return parsed;

        if (profile.Type == ModLaunch.McLoginType._4399)
            return OpenNelLoginKind.Login4399Password;

        if (profile.Type != ModLaunch.McLoginType.NetEase)
            return OpenNelLoginKind.Unknown;

        var account = profile.Name ?? profile.Username ?? "";
        if (string.IsNullOrWhiteSpace(profile.Password))
            return LooksLikePhone(account) ? OpenNelLoginKind.NeteasePhone : OpenNelLoginKind.NeteaseCookie;

        return account.Contains("@", StringComparison.Ordinal)
            ? OpenNelLoginKind.NeteaseEmail
            : OpenNelLoginKind.NeteaseEmail;
    }

    private static string GetCommunityDetailsJson(McProfile profile)
    {
        if (!string.IsNullOrWhiteSpace(profile?.CommunityDetailsJson))
            return profile.CommunityDetailsJson;
        if (profile is null)
            return "";

        return GetCommunityLoginKind(profile) switch
        {
            OpenNelLoginKind.Login4399Password => JsonSerializer.Serialize(new
            {
                Account = profile.Name ?? profile.Username ?? "",
                Password = profile.Password ?? ""
            }),
            OpenNelLoginKind.NeteaseEmail => JsonSerializer.Serialize(new
            {
                email = profile.Name ?? profile.Username ?? "",
                password = profile.Password ?? ""
            }),
            OpenNelLoginKind.NeteasePhone => JsonSerializer.Serialize(new
            {
                phone = profile.Name ?? profile.Username ?? ""
            }),
            OpenNelLoginKind.NeteaseCookie => profile.AccessToken ?? "",
            _ => ""
        };
    }
